...
void FileController::mkdir(const QString &name)
{
    ...
    QString path = (m_currentPath.endsWith('/') ? m_currentPath : m_currentPath + "/") + name.trimmed();
    setBusy(true);
    setError({});
 ...
 */
void FileController::uploadFiles(const QStringList &localPaths)
{
